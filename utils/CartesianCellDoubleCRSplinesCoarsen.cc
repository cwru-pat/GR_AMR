 ...
 *
 ************************************************************************/
#include "CartesianCellDoubleCRSplinesCoarsen.h"
...
CartesianCellDoubleCRSplinesCoarsen::CartesianCellDoubleCRSplinesCoarsen():
  hier::CoarsenOperator("CRSPLINES_COARSEN")
{
...
int
CartesianCellDoubleCRSplinesCoarsen::getOperatorPriority() const
{
   return 0;
}

hier::IntVector
CartesianCellDoubleCRSplinesCoarsen::getStencilWidth(const tbox::Dimension& dim) const
{
  return hier::IntVector(dim, 1);
}

void CartesianCellDoubleCRSplinesCoarsen::computeCoef(
...
double CartesianCellDoubleCRSplinesCoarsen::evaluateInterpolation(
...
void
CartesianCellDoubleCRSplinesCoarsen::coarsen(
   hier::Patch& coarse,
   ...
   const hier::IntVector& ratio) const
{
         ...
         for(int i = ifirstc[0]; i <= ilastc[0]; i++)
         {
           ...
           for(int si = 0; si < 4; si++)
             for(int sj = 0; sj < 4; sj++)
               ...
               {
                 f[si*16 + sj*4 + sk] = farray(il-1+si, jl-1+sj, kl-1+sk);
               }
           computeCoef(a, f);

           carray(i, j, k) = evaluateInterpolation(a, id, jd, kd);
   ...
   {
     TBOX_ERROR("CartesianCellDoubleCRSplinesCoarsen error...\n"

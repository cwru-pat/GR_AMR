 ...
 *
 ************************************************************************/
#include "CartesianCellDoubleCRSplinesRefine.h"
...
CartesianCellDoubleCRSplinesRefine::CartesianCellDoubleCRSplinesRefine():
  hier::RefineOperator("CRSPLINES_REFINE")
{
...
void CartesianCellDoubleCRSplinesRefine::computeCoef(
...
double CartesianCellDoubleCRSplinesRefine::evaluateInterpolation(
...
int
CartesianCellDoubleCRSplinesRefine::getOperatorPriority() const
{
   return 0;
}
...
hier::IntVector
CartesianCellDoubleCRSplinesRefine::getStencilWidth(const tbox::Dimension& dim) const
{
  return hier::IntVector(dim, 2);
}

void
CartesianCellDoubleCRSplinesRefine::refine(
   hier::Patch& fine,
   ...
   const hier::IntVector& ratio) const
{
         ...
         for(int i = ifirstf[0]; i <= ilastf[0]; i++)
         {
           ...
           for(int si = 0; si < 4; si++)
             for(int sj = 0; sj < 4; sj++)
               ...
               {
                 f[si*16 + sj*4 + sk] = carray(il-1+si, jl-1+sj, kl-1+sk);
               }
           computeCoef(a, f);

           farray(i, j, k) = evaluateInterpolation(a, id, jd, kd);
         }
       }
     }
   }
   ...
   {
     TBOX_ERROR("CartesianCellDoubleCRSplinesRefine error... \n"

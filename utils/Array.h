...
#include <string>
#include <utility>
#include "../components/IO/HDFIO/src/H5IO.h"
...
class CosmoArray
{
  public:
    ...
    bool writeToFile(H5IO & io, std::string file_name)
    {
      return io.writeArrayToFile(_array, file_name.c_str(), name.c_str(), false);
    }

    bool readFromFile(H5IO & io, std::string file_name)
    {
      return io.readArrayFromFile(_array, file_name.c_str(), name.c_str());
    }
};

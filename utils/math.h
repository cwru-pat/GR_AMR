...
{
  if( c > 0) return c * forward_derivative(i,j,k,d,field);
  else return c * backward_derivative(i,j,k,d,field);
}
    ...
    arr_t & field)
{
  switch (d) {
    case 1:
      if( (NX-i-1) < bd_nx/2 )
        return forward_derivative(i,j,k,d,field);
      else
        return backward_derivative(i,j,k,d,field);
      break;
    case 2:
      if( (NY-j-1) < bd_ny/2 )
        return forward_derivative(i,j,k,d,field);

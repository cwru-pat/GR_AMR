...
#define BSSN_SWAP(field, pcellmath)  \
  pcellmath##.swapData(patch, field##_p_idx, field##_f_idx)  \ 
...
#define BSSN_DT_GAMMAI_NOSHIFT(I) ( \
    ...
    + 2.0*bd->alpha*( \
        ...
        - (1.0/3.0) * ( \
            ...
            + 4.0*bd->gammai##I##1*bd->d1theta + 4.0*bd->gammai##I##2*bd->d2theta + 4.0*bd->gammai##I##3*bd->d3theta \
          ) \
        - 1.0*bd->alpha*Z4c_K1_DAMPING_AMPLITUDE*( \

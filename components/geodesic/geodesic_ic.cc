...
namespace cosmo
{
    ...
    BSSN *bssn)
  {
    ...
    bool do_extrapolation = cosmo_geodesic_db->getBoolWithDefault("do_extrapolation", false);
    ...
    std::ifstream infile("healpix_data_NSIDE_8.dat");
    std::string line;
    ...
    for(int i = 0; i < N_origins; i++)
    {
      ...
      //      ids[i].resize(healpy_data[0].size);
      for(int j = 0; j < healpy_data[0].size(); j++)
        ids[i].push_back(j + i * (int)healpy_data[0].size() + num_p);
    }
    
    geodesic_ic_set_bundles(
      hierarchy,
      origins_x, origins_y, origins_z, dirs_x, dirs_y, dirs_z, bssn, epsilon, ids);

    int delta_num_p = ids[N_origins-1].back() - num_p + 1;
    num_p = ids[N_origins-1].back()+1;
    if(do_extrapolation)
    {
      for(int i = 0; i < N_origins; i++)
        for(int j = 0; j < healpy_data[0].size(); j++)
      ...
      geodesic_ic_set_bundles(
        hierarchy,
        origins_x, origins_y, origins_z, dirs_x, dirs_y, dirs_z, bssn, epsilon / 2.0, ids);
      ...
      geodesic_ic_set_bundles(
        hierarchy,
        origins_x, origins_y, origins_z, dirs_x, dirs_y, dirs_z, bssn, epsilon / 4.0, ids);

      num_p = ids[N_origins-1].back() + 1;
    }
    
    printAll(hierarchy, pc_idx);
    tbox::pout<<"Have inserted "<<num_p<<" particles!\n";
  }
              ...
              && k0 >= lower[2] && k0 <= upper[2])
          {
            ...
            //            for(int d_idx; d_idx < dir_x[o_idx].size(); d_idx++)
            {
              geodesic_ic_transform_inertial_vectors(
                ...
                dirs_x[o_idx], dirs_y[o_idx], dirs_z[o_idx], dx, epsilon, ids[o_idx]);
...
void Geodesic::geodesic_ic_face_null_test(
    ...
    std::shared_ptr<tbox::Database> cosmo_geodesic_db)
  {
    ...
    p_info[4] = eps, p_info[5] = 0, id[0] = 3*num_p+1;
    input_p[1] = new RKParticle(p_info, id);

    p_info[5] = eps, p_info[4] = 0, id[0] = 3*num_p+2;
